...
angular.module('ng-app', [])
  ...
  .controller('MainCtrl', function($scope, $timeout, $window) {
    ...
    $scope.userType = null;
    $scope.Applyback = function(value){
      if($scope.userType){
        $("#back").css("background","url("+value+")");
        $("#back").css("background-size","cover");
        $(".back-affect").css("opacity","0.5");
      }
    ...
    if($scope.userType){
          $scope.Applyback($scope.userType); 
      }
    else{
      COLORS.BOARD  = "#000";
    }
    ...
    $scope.score = 0;
    //Changing the style of the board after every game
    ...
    //Update function which controls every move
    function update() {
    ...
    //Defining functions of snakes actions
    ...
    //Redrawing board after game over
    function gameOver() {
      isGameOver = true;
